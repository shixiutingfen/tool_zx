...
class Factory:
    ...
    def create(self, _type, signalObj, *args, **kwargs):
        """
        Create a new object.
        ...
        if _type not in self._items:
            # print "=== type {} not in {}".format(_type, self._items)   
            return None
        item = self._items[_type]
        # LOG.info("Create item with type {} item {}...".format(_type, item))
        if item is None:
            return None
        return item(_type, signalObj, *args, **kwargs)
